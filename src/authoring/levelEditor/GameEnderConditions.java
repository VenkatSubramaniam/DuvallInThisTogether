...
public class GameEnderConditions extends VBox {
	...
	private GameHealthSelector healthManager;
	...
	private final String DEFEAT_HEALTH_CONDITIONS = "healthAmount";
	...
	private void record() {
		...
		List<Integer> selectedPointLevels = new ArrayList<>();
		List<Integer> selectedTimeLevels = new ArrayList<>();
		List<Integer> selectedHealthLevels = new ArrayList<>();
		for(int i = 0; i<myController.getNumLevelsForGame(); i++) {
			...
			if (checkBoxes.get(i).isSelected()) {
				...
				if(defeat.getValue()!=null) {
					...
					if(defeat.getValue().equals(PropertiesGetter.getProperty(DEFEAT_HEALTH_CONDITIONS))) {
						selectedHealthLevels.add(Integer.parseInt(checkBoxes.get(i).getText()));
					}
				}
		...
		if (selectedHealthLevels.size()>0) {
			healthManager.createCheckBoxes(selectedHealthLevels);
			healthManager.show();
		}
	...
	public void setHealthManager(GameHealthSelector health) {
